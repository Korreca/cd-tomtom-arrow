...
use super::renderer::{InfoOverlay, OVERLAY_SIZE, draw_isometric_arrow, render_layered_frame};
...
use windows::Win32::Graphics::Gdi::{
    ...
    DeleteObject, EndPaint, FillRect, GRAPHICS_MODE, HBRUSH, HGDIOBJ, PAINTSTRUCT, SRCCOPY,
    SelectObject, SetGraphicsMode,
};
use windows::Win32::System::Threading::GetCurrentProcessId;
use windows::Win32::UI::Input::KeyboardAndMouse::{ReleaseCapture, SetCapture};
use windows::Win32::UI::WindowsAndMessaging::{
    CREATESTRUCTA, CS_HREDRAW, CS_VREDRAW, CreateWindowExA, DefWindowProcA, DestroyWindow,
    FindWindowA, GWL_EXSTYLE, GetClientRect, GetWindowLongPtrA, GetWindowRect,
    GetWindowThreadProcessId, HCURSOR, HICON, HWND_TOP, PostMessageA, PostQuitMessage,
    RegisterClassA, SW_HIDE, SW_SHOW, SWP_NOSIZE, SWP_NOZORDER, SetTimer, SetWindowLongPtrA,
    SetWindowPos, ShowWindow, WINDOW_LONG_PTR_INDEX, WM_CLOSE, WM_CREATE, WM_DESTROY,
    WM_LBUTTONDOWN, WM_LBUTTONUP, WM_MOUSEMOVE, WM_PAINT, WM_TIMER, WNDCLASSA, WS_EX_LAYERED,
...
impl OverlayWindow {
    ...
    pub fn initialize(&mut self) -> Result<(), String> {
        unsafe {
            let class_name =
                CString::new("CrimsonDesertArrowOverlay").map_err(|_| "Class name error")?;

            // Destroy any zombie window from a previous run (cross-process FindWindowA)
            if let Ok(existing_hwnd) =
                FindWindowA(PCSTR(class_name.as_ptr().cast::<u8>()), PCSTR::null())
            {
    ...
    match msg {
        ...
        WM_PAINT => {
            ...
            if !hdc.0.is_null() {
                ...
                unsafe {
                    FillRect(hdc_mem, &raw const rect_mem, hbrush);
                }
                unsafe {
                ...
                unsafe {
                    SetGraphicsMode(hdc_mem, GRAPHICS_MODE(2));
                } // GM_ADVANCED = 2

                // Get overlay window state
                let overlay_ptr = unsafe { GetWindowLongPtrA(hwnd, WINDOW_LONG_PTR_INDEX(0)) }
                    as *mut OverlayWindow;
                ...
                let (scale, text_scale) = if overlay_ptr.is_null() {
                    (1.0, 1.0)
                } else {
                    unsafe { &*overlay_ptr }
                        .config
                ...
                let (distance, height_diff, text_offset_x, text_offset_y) = if overlay_ptr.is_null()
                {
                    (0.0, 0.0, 0, 0)
                } else {
                    ...
                    let (tx, ty) = ov
                        .config
                        .lock()
                        .map_or((0, 0), |c| (c.text_offset_x, c.text_offset_y));
        ...
        WM_LBUTTONDOWN => {
            ...
            let overlay_ptr =
                unsafe { GetWindowLongPtrA(hwnd, WINDOW_LONG_PTR_INDEX(0)) } as *mut OverlayWindow;
        ...
        WM_TIMER => {
            ...
            if !overlay_ptr.is_null() {
                    ...
                    && let Ok(mut app_state) = app_state_arc.lock()
                {
                    ...
                    if marker_changed || user_reset {
                        ...
                        overlay.last_nav_signature =
                            (overlay.turn_deg, overlay.distance, overlay.height_diff);
                    }
                ...
                // Evaluate hide/show rules
                {
                    ...
                    if overlay.visible {
                        ...
                        let below_threshold =
                            cfg.hide_below_m > 0 && distance < cfg.hide_below_m as f32;
                        if below_threshold {
            ...
            let is_hidden = if overlay_ptr.is_null() {
                true
            } else {
            ...
            if !overlay_ptr.is_null() {
                ...
                let (
                    ...
                    info_offset_y,
                    info_scale,
                ) = if let Ok(cfg) = overlay_ref.config.lock() {
                    (
                        cfg.opacity,
                        cfg.scale,
                        cfg.text_scale,
            ...
            unsafe {
                let _ = ShowWindow(hwnd, if is_hidden { SW_HIDE } else { SW_SHOW });
            }
            LRESULT(0)
        }
